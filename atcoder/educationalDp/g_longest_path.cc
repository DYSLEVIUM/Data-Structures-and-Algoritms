...
inline void solve() {
  ...
  auto dfs = [&gr, &dp](const auto& dfs, const ll& node, const ll& len) -> void {
    //  already reached earlier
    if (dp[node] != INT_MIN) return;

    //  longest length from the this node is atleast 0, i.e., if we start from this node
    dp[node] = 0;

    for (auto neighbor : gr[node]) {
      //  get all other length of neighbors from node
      dfs(dfs, neighbor, len + 1);

      //  max length of the node will be 1 + max length of reaching the neighbor
      dp[node] = max(dp[node], dp[neighbor] + 1);
    }
  };

  //  start the dfs from the ith node where reqaching it is 0
  fo(i, n) dfs(dfs, i, 0);

  //  get the max from length from considering all nodes as starting point
