...
inline void solve() {
    ...
    fo(i, n + 1)
        ...
        if (i == 0 || j == 0)
            ...
            dp[i][j] = max(dp[i - 1][j], dp[i - 1][j - W[i - 1]] + V[i - 1]);  //  getting max of either previous state that is without adding anything in the knapsack, or previous state when weight of the bag was (j-W[i-1]) + V[i-1] if current element is added to knapsack
        else
