...
inline void solve() {
  ...
  ll lcs_length = dp[n][m];

  string ans(lcs_length, '@');

  ll i = n, j = m;
  while (i > 0 && j > 0) {
    if (s[i - 1] == t[j - 1]) {
      ans[lcs_length - 1] = s[i - 1];
      --i, --j, --lcs_length;
    }
    //  if the character's don't match, we go to the substring where the lcs is greater, i.e., string of (0->i-1 & 0->j) or (0->i & o->j-1) whichever will give the greater value
