...
inline void solve() {
  //  minimum absolute difference in a sorted array
  vl arr{4, 5, 8};
  ll key = 7;
  // vl arr{1, 3, 8, 10, 15};
  // ll key = 12;
  ...
  if (ans != 0) {  //  element is not present in the array, the minimum will be the neighbouring elements if the element would have been present
    if (abs(key - arr[lo]) < abs(key - arr[hi]))
      ans = arr[lo];
    else
      ans = arr[hi];
  }
