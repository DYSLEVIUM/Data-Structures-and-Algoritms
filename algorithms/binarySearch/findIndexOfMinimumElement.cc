...
inline void solve() {
  //  given: an array sorted in ascending order is rotated some arbitary number of times, we have to return the number of times it was rotated
  // vl ar{11, 12, 15, 18, 2, 5, 6, 8};
  vl ar{2, 5, 6, 8, 11, 12, 15, 18};

  //  index of the minimum element will be the number of times the array is rotated
  ll ans = -1;
  ...
  while (lo <= hi) {
    ...
    if (lo == hi) {
      ans = mid;
      break;
    } else if (ar[mid] < ar[hi]) {  //  right side is sorted
      hi = mid - 1;
    } else {
      lo = mid + 1;
    }
