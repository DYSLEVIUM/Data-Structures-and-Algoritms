...
inline void solve() {
  //  allocate number of pages so that the maximum total pages is minimum
  vl arr{10, 20, 30, 40};
  ll k = 3;

  //  conditions
  //  1. each student should have at least one book
  //  2. books can't be shared
  //  3. assignment of books should be continuous
  ...
  auto isValid = [](const vl& arr, const ll& k, const ll& currMax) {
    ll cnt = 1;  //  at least one student
    ll sum = 0;
  ...
  ll lo = maxi;  //  one book to one student
  ll hi = sum;   //  all other to other
