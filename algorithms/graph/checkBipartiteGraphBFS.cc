...
inline void solve() {
  ...
  adjList2.eb(vl{1});
  adjList2.eb(vl{0, 2, 7});
  adjList2.eb(vl{1, 3});
  adjList2.eb(vl{2, 4});
  adjList2.eb(vl{3, 5, 7});
  adjList2.eb(vl{4, 6});
  adjList2.eb(vl{5});
  adjList2.eb(vl{1, 4});

  auto isBipartite = [](const vvl& gr) {
    vl color(gr.size(), -1);  //  initially no color

    auto bfsCheck = [&gr, &color](const ll& node) {
      queue<ll> qu;

      //  initial node with color 0
      qu.push(node);
      color[node] = 0;
      ...
      while (!qu.empty()) {
        ...
        for (auto neighbour : gr[nd]) {
          if (color[neighbour] == -1) {
            color[neighbour] = 1 - color[nd];
            qu.push(neighbour);
          } else {
            if (color[neighbour] == color[nd]) return false;
          }
        }
      }
    ...
    //  done so that multiple components are covered
    fo(i, gr.size()) {
      if (color[i] == -1) {
  ...
  cout << isBipartite(adjList1) << '\n';
  cout << isBipartite(adjList2) << '\n';
}
