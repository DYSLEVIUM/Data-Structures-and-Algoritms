...
inline void solve() {
  // a node is a articulation point (cut vertex) if on the removal of the node (and its associated edges), the number of components increases

  //  tarjan's algorithm to find articulation points O(V+E)

  //  case 1: if node U is root of DFS tree and has atleast 2 children (subgraphs)

  //  case 2: if node U is not a root of DFS tree and it has a child V such that no vertex in the subtree rooted with V has a back edge to one of the ancestors of U (excluding U, i.e., parent of V doesn't count as we assume that U is being removed)

  ll const N = 5;

  vvl adj(N);  //  undirected graph
  ...
  vl AP(N, false);  //  initially, none of them are articulation points

  auto find_APs_tarjan = [&AP, &N](const vvl& gr) {
    ...
    auto dfs = [&](const auto& dfs, const ll& u) -> void {
      ...
      for (auto v : gr[u]) {
        if (disc[v] == -1) {
          ...
          if (parent[u] != -1 && low_link[v] >= disc[u]) AP[u] = true;
        }
        //  ignoring child to parent edge
        else if (v != parent[u])  //  this is a back edge, so we are able to reach a previously visited node
