...
inline void solve() {
  ...
  auto isCyclic = [](const vvl& gr) {
    ...
    //  to check if a node has been visited in the current dfs call stack
    vector<bool> dfsVisited(gr.size(), false);

    auto dfs = [&gr, &visited, &dfsVisited](const ll& node, const auto& dfs) -> bool {
      visited[node] = true;
