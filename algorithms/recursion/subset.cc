...
//  print all subsets
vvi subset;
void generateSubset(vi ip, vi op) {
    if (ip.size() == 0) {
        subset.eb(op);
        return;
    }
    ...
    vi op1 = op;
    op1.eb(ip[ip.size() - 1]);  //  taking the last element in output
    ip.pop_back();
    generateSubset(ip, op1);

    vi op2 = op;  //  not taking the last element in output
    generateSubset(ip, op2);
}
