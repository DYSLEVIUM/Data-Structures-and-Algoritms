...
void generateBalancedParantheses(ll open, ll close, string op) {
    if (!open && !close) {
        cout << op << '\n';
        return;
    }
    ...
    if (open) {
        generateBalancedParantheses(open - 1, close, op + '(');
    }

    if (open < close) {
        generateBalancedParantheses(open, close - 1, op + ')');
    }
}

inline void solve() {
    int n = 10;
    generateBalancedParantheses(n, n, "");
}
