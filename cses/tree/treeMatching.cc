...
inline void solve() {
  ...
  //  dp solution
  // vvl dp(n, vl(2, -1));
  // auto dfs = [&gr, &dp](const auto& dfs, const ll& u, const ll& parent) -> ll
  // {
  //   if (dp[u][0] != -1 && dp[u][1] != -1) return max(dp[u][0], dp[u][1]);

  //   dp[u][0] = 0;
  //   dp[u][1] = 0;

  //   //  don't choose the node
  //   for (auto& node : gr[u]) {
  //     if (node == parent) continue;
  //     dp[u][0] += dfs(dfs, node, u);
  //   }

  //   for (auto& node : gr[u]) {
  //     if (node == parent) continue;

  //     ll all_other = dp[u][0] - max(dp[node][0], dp[node][1]);
  //     dp[u][1] = max(dp[u][1], 1 + dp[node][0] + all_other);
  //   }

  //   return max(dp[u][0], dp[u][1]);
  // };

  //  greedily taking all the nodes from the leaf upto the root which hasn't
  //  been used yet
  vector<bool> visited(n, false);
  auto dfs = [&gr, &visited](const auto& dfs, const ll& u,
                             const ll& parent) -> ll {
    ll cnt = 0;
    for (auto& node : gr[u]) {
      if (node == parent) continue;
    ...
    if (!visited[u] && (parent != -1 && !visited[parent])) {
      ++cnt;
      visited[u] = visited[parent] = true;
    }

    return cnt;
  };

  //  rooting at 0 with no parent
  cout << dfs(dfs, 0, -1);
}
