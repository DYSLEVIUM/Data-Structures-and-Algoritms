...
inline void solve() {
  ...
  vector<Vector2d<ll>> vertices(n);
  cin >> vertices;

  auto is_intersecting_segments = [](vector<Vector2d<ll>> points) {
    fo(i, 2) {
      ll cross_1_sign = points[0].area(points[2], points[3]);
      ll cross_2_sign = points[1].area(points[2], points[3]);

      if ((cross_1_sign < 0 && cross_2_sign < 0) ||
          (cross_1_sign > 0 && cross_2_sign > 0))
        return false;
    ...
    Vector2d<ll> line_1(points[1] - points[0]), line_2(points[3] - points[2]);

    if (line_1.get_y() * line_2.get_x() == line_2.get_y() * line_1.get_x()) {
      if (points[0].area(points[1], points[2]) != 0) return false;

      fo(i, 2) {
        if (max(points[0].get_x(), points[1].get_x()) <
                min(points[2].get_x(), points[3].get_x()) ||
            max(points[0].get_y(), points[1].get_y()) <
                min(points[2].get_y(), points[3].get_y()))
          return false;
  ...
  auto segment_contains_point = [](const Vector2d<ll>& vert_1,
                                   const Vector2d<ll>& vert_2,
                                   const Vector2d<ll>& point) {
    if (vert_1.area(vert_2, point)) return false;  //  not collinear

    //  check if the point is between the vertices using bounding box;
    return min(vert_1.get_x(), vert_2.get_x()) <= point.get_x() &&
           point.get_x() <= max(vert_1.get_x(), vert_2.get_x()) &&
           min(vert_1.get_y(), vert_2.get_y()) <= point.get_y() &&
           point.get_y() <= max(vert_1.get_y(), vert_2.get_y());
  };

  //  we make a infinite ray and whenever we get odd intersections, we know that
  //  the point is inside the polygon
  fo(i, m) {
    Vector2d<ll> point;
    cin >> point;

    Vector2d<ll> ray(point.get_x() + 1, INT_MAX);
    ll cnt = 0;
    bool is_on_boundary = false;
    fo(i, n) {
      Vector2d<ll> p = vertices[i];
      Vector2d<ll> q = vertices[i + 1 < n ? i + 1 : 0];
      if (segment_contains_point(p, q, point)) {
        is_on_boundary = true;
        break;
      }
      cnt += is_intersecting_segments(vector<Vector2d<ll>>{point, ray, p, q});
    }
    if (is_on_boundary)
      cout << "BOUNDARY\n";
    else if (cnt & 1)
