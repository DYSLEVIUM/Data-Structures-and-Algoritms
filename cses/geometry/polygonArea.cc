...
inline void solve() {
  ...
  vector<Vector2d<ll>> verticies(n);
  cin >> verticies;

  ll area = 0;

  //  method 1: using the area of trapezoid
  // fo(i, n) {
  //   Vector2d<ll> p = verticies[i];
  //   Vector2d<ll> q = i ? verticies[i - 1] : verticies.back();
  //   //  getting area bounded by each edge and y-axis
  //   area += (p.get_x() - q.get_x()) * (p.get_y() + q.get_y());
  // }
