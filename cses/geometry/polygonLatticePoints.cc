...
class Vector2d {
 ...
 public:
  ...
  bool operator!=(const Vector2d<T>& vec) const { return !(this == vec); }
...
inline void solve() {
  ...
  fo(i, n) {
    ...
    Vector2d<ll> vect = p - q;
    boundary += gcd(abs(vect.get_x()), abs(vect.get_y()));
  }

  //  pick's theorem to get the lattice points inside the polygon
  //  area = (lattice points inside) + (boundary lattice points)/2 - 1
  ll area = 0;
  Vector2d<ll> ref;
  fo(i, n) {
    Vector2d<ll> p = vertices[i];
  ...
  //  multiplying by 2 so that there is no precision error while subracting
  ll inside = area + 2 - boundary;
  inside >>= 1;
