...
inline void solve() {
  vector<Vector2d<ll>> points(3);
  cin >> points[0] >> points[1] >> points[2];

  // change of origin w.r.t point1
  Fo(i, 1, 3) points[i] -= points[0];

  ll cross = points[1].get_x() * points[2].get_y() -
             points[1].get_y() * points[2].get_x();
