...
inline void solve() {
  ...
  vector<Vector2d<ll>> points(n);
  cin >> points;

  sort(all(points), [](const Vector2d<ll>& a, const Vector2d<ll>& b) {
    return make_pair(a.get_x(), a.get_y()) < make_pair(b.get_x(), b.get_y());
  ...
  fo(i, 2) {
    ...
    //  getting the bottom part of the convex hull
    reverse(all(points));
    //  the last point will be added twice, so removing it
    convex_hull.pop_back();
  }

  cout << convex_hull.size() << '\n';
  for (auto& convex_hull_point : convex_hull) cout << convex_hull_point << '\n';
}
