...
inline void solve() {
  ...
  while (!qu.empty()) {
    ...
    for (auto& neighbour : gr[fr]) {
      if (min_dist[neighbour] > min_dist[fr] + 1) {
        qu.push(neighbour);
        min_dist[neighbour] = min_dist[fr] + 1;
      }
    }
  }

  if (min_dist[n - 1] == INT_MAX) {
    cout << "IMPOSSIBLE";
    return;
  }
  ...
  while (node != 0) {
    ...
    for (auto& neighbour : gr[node]) {
      if (min_dist[neighbour] < minn) {
        minn = min_dist[neighbour];
        new_node = neighbour;
      }
    }
