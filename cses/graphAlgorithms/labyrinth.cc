...
inline void solve() {
  ...
  ll st_row = -1, st_col = -1;
  ll ed_row = -1, ed_col = -1;
  ...
  fo(row, rows) {
    fo(col, cols) {
      ...
      if (mat[row][col] == 'B') {
        ed_row = row, ed_col = col;
      }
    }
  }

  auto check = [&](const ll& row, const ll& col) {
  ...
  queue<pl> qu;
  vvl min_dist(rows, vl(cols, INT_MAX));
  min_dist[st_row][st_col] = 0;

  vpl delta{{-1, 0}, {1, 0}, {0, -1}, {0, 1}};
  qu.push({st_row, st_col});
  while (!qu.empty()) {
    pl curr_node = qu.front();
    qu.pop();

    fo(i, 4) {
      ll curr_row = curr_node.F + delta[i].F,
         curr_col = curr_node.S + delta[i].S;
      if (check(curr_row, curr_col)) {
        if (min_dist[curr_row][curr_col] >
            1 + min_dist[curr_node.F][curr_node.S]) {
          qu.push({curr_row, curr_col});
          min_dist[curr_row][curr_col] = 1 + min_dist[curr_node.F][curr_node.S];
        }
      }
    }
  }

  if (min_dist[ed_row][ed_col] == INT_MAX) {
    cout << "NO";
    return;
  }

  cout << "YES\n";
  cout << min_dist[ed_row][ed_col] << '\n';

  string path = "";

  vector<char> delta_2{'D', 'U', 'R', 'L'};
  while (mat[ed_row][ed_col] != 'A') {
    ll minn = INT_MAX;
    ll delta_min = -1;
    fo(i, 4) {
      if (check(ed_row + delta[i].F, ed_col + delta[i].S)) {
        if (min_dist[ed_row + delta[i].F][ed_col + delta[i].S] < minn) {
          minn = min_dist[ed_row + delta[i].F][ed_col + delta[i].S];
          delta_min = i;
        }
      }
    }
    ed_row += delta[delta_min].F;
    ed_col += delta[delta_min].S;

    path += delta_2[delta_min];
  }
