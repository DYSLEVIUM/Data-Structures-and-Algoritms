...
class DSU {
   ...
   public:
    ...
    T find(const T& node) {
        //  path compression
        return this->parent[node] = this->parent[node] == node
                                        ? node
    ...
    void merge(T node_1, T node_2) {
        ...
        this->rank[node_1] < this->rank[node_2] ? this->parent[node_1] = node_2
                                                : this->parent[node_2] = node_1;
    }
};
...
inline void solve() {
    ...
    cout << sz(candidates) - 1 << '\n';
    for (ll i = 1; i < sz(candidates); ++i) {
        cout << candidates[0] + 1 << ' ' << candidates[i] + 1 << '\n';
    }
}
