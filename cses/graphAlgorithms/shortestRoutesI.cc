...
inline void solve() {
  ...
  //  we need to perform single source shortest path (sssp), we will use
  //  dijkstra's algorithm which has a time complexity of O(elogv)

  auto dijkstra = [&](const ll& start) {
    vl dist(n, LONG_LONG_MAX);  //  reaching the node is infinity
    dist[start] = 0;            //  reaching the start is 0

    //  min heap of {min distance to reach, node} pair
    priority_queue<pl, vpl, greater<pl>> min_heap;
    min_heap.push({0, start});
    while (!min_heap.empty()) {
      pl minn = min_heap.top();
      min_heap.pop();

      ll minn_dist = minn.F;
      ll minn_node = minn.S;

      //  if we can already reach this node with a better value we don't need to
      //  do anything
      if (dist[minn_node] < minn_dist) continue;

      //  if we get a better distance for this node, we propagate this to its
      //  neighbours and we add it to the minheap for a possiblity of better
      //  distance for its neighbours
      for (pl& edge : gr[minn_node]) {
        ll new_dist = dist[minn_node] + edge.S;

        if (new_dist < dist[edge.F]) {
          dist[edge.F] = new_dist;
          min_heap.push({new_dist, edge.F});
        }
      }
    }
    return dist;
