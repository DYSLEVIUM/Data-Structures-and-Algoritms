...
inline void solve() {
  ...
  //  we need to perform all-pairs shortest path (apsp), we will use
  //  floyd-warshall algorithm which has a time complexity of O(v^3)

  auto floyd_warshall = [&]() {
    vvl dist(n, vl(n, LONG_LONG_MAX / 2));

    //  as there are no self loops
    fo(i, n) { dist[i][i] = 0; }

    fo(i, n) {
      fo(j, gr[i].size()) {
        dist[i][gr[i][j].F] = min(dist[i][gr[i][j].F], gr[i][j].S);
      }
    }

    fo(k, n) {
      fo(i, n) {
        fo(j, n) { dist[i][j] = min(dist[i][j], dist[i][k] + dist[k][j]); }
      }
    }

    return dist;
  };
  vvl dist = floyd_warshall();
