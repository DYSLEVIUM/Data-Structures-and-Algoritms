...
inline void solve() {
  ...
  auto is_cycle = [&](const auto& is_cycle, const ll& node) {
    ...
    for (auto& neighbour : gr[node]) {
      if (is_cycle(is_cycle, neighbour)) return true;
    }
    dfs_vis[node] = false;
  ...
  fo(i, n) vis[i] = false;
  auto top_sort = [&](const auto& top_sort, const ll& node) -> void {
    if (vis[node]) return;

    vis[node] = true;
    for (auto& neighbour : gr[node]) {
      top_sort(top_sort, neighbour);
    }
    cout << node + 1 << ' ';
  };
