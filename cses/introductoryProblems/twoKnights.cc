...
inline void solve() {
  /*
    ...
    forms either a 2×3 or 3×2 board. And for each of 2×3 and 3×2 boards, there
    are 2 ways of placing two knights so that they threaten each other. So,
    what we should do is to count how many 2×3 and 3×2 squares on n×n board.
    ...
    2⋅2(n−1)(n−2)=4(n−1)(n−2) ways of placing two knights so that they threaten
    each other. So what we are looking for is n^2(n^2−1)/2−4(n−1)(n−2). It is
    also worth mentioning that we are not over-counting because whenever we
    place two knights so that they threaten each other, either a 2×3 or 3×2
  ...
  */
  ...
  Fo(k, 1, n + 1) {
    ll first_knight = k * k, second_knight = k * k - 1;
    ll total_ways = first_knight * second_knight /
                    2;  //  divide by 2! as both knights are identical

    //  removing the ways that they attack each other
