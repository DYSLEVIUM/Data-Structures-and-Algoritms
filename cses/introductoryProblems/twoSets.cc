...
inline void solve() {
  ...
  ll curr_num = n;
  ll curr_sum = 0;
  //  we keep on adding the largest numbers, and when the sum in one set becomes
  //  larger than target, we break and add the remaining as we have numbers from
  //  1 ... x, remaining
  while (curr_sum + curr_num < target) {
    se.insert(curr_num);
    curr_sum += curr_num;
    --curr_num;
  }
  if (curr_sum != target) {
    se.insert(target - curr_sum);
  }
