...
inline void solve() {
  ...
  fo(i, n) {
    ...
    //  we see if pre_sum - x has been seen before or not, so we add the total
    //  subarrays that can be formed using these
    if (mp.find(pre_sum - x) != mp.end()) ans += mp[pre_sum - x];

    //  adding this subarray to hashmap
    ++mp[pre_sum];
  }
  cout << ans << '\n';
}
