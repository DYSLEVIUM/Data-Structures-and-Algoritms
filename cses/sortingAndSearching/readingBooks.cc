...
inline void solve() {
  ...
  //  it will always take total time to read the books so as to alternate
  //  between two indiviuals; the only edge case is that if the largest book
  //  takes more than or equal to 2 * time to finish than, we cannot alternate
  //  the books and have to wait for the largest book to finish
  ll sum = accumulate(all(book_time), 0LL);
  cout << max(sum, book_time[n - 1] * 2);
}
