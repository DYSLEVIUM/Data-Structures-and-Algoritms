...
inline void solve() {
  ...
  pbds_set<ll> children;
  Fo(i, 1, n + 1) children.insert(i);

  ll start_pos = 0;
  while (!children.empty()) {
    start_pos %= children.size();
    ll pos_to_remove = (start_pos + k) % children.size();

    auto child = children.find_by_order(pos_to_remove);

    cout << *child << ' ';
    children.erase(child);

    start_pos = pos_to_remove;
  }
}
