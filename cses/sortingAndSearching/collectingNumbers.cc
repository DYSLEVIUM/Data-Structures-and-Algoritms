...
inline void solve() {
  ...
  //  we only care about any number x and x + 1, if x + 1 comes before x, we
  //  need to make at one more round to take both the numbers

  ll ans = 1;  //  at least 1 round to collect all the numbers
  pbds_unordered_map<ll, ll> mp;
  fo(i, n) {
    ll x;
    cin >> x;

    if (mp.find(x + 1) != mp.end() && mp[x + 1] < i + 1) ++ans;
    mp[x] = i + 1;
  }
