...
inline void solve() {
  ...
  while (children.size() != 1) {
    ...
    //  if the curr_size was even, new problem's array is the same, otherwise we
    //  start from the last element
    if (curr_size % 2 == 0)
      children = survivors;
    else {
      ll start = survivors.back();
      survivors.pop_back();
      children.clear();
      children.pb(start);
      for (auto& x : survivors) children.pb(x);
    }
  }
