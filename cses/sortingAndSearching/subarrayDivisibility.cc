...
inline void solve() {
  ...
  fo(i, n) {
    ...
    //  doing double modulo so as to remove the negative sign
    ++mp[((pref_sum % n) + n) % n];
  }

  //  adding all the subarryas that are completely divisible by n;
  ll ans = mp[0];

  //  the total subarryas will be the kC2 of the current remainder, as the same
  //  remainder would have only appeared if the offset was k; where k is the
  //  dividend
  fo(i, n) { ans += mp[i] * (mp[i] - 1) / 2; }
