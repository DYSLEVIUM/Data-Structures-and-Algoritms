...
inline void solve() {
  ...
  //  now we want to able to make coin sum k + 1

  //  let the coin at position i be num, so there are two cases:

  //  case 1: num > k + 1 -> there is a gap, i.e we can make the range [1, k]
  //  and [1+(k+1), k+(k+1)], we are not able to make k + 1

  //  case 2: num <= k + 1
  //  -> we can make the range [1, k] and [1+(num), k+(num)], here 1 + num will
  //  lie partially in the range of [1, k], so our equivalent range comes out to
  //  be [1, k + num]
