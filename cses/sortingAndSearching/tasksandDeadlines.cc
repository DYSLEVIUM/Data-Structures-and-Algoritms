...
inline void solve() {
  ...
  //  here the task that is taken first will be repeated the most in prefix to
  //  be subtracted, so we try to minimize that by taking the smallest element
  //  the most
  sort(all(tasks));

  //  now we just simulate the process
  ll time = 0, reward = 0;
