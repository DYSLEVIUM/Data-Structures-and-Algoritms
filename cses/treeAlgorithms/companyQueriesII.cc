...
inline void solve() {
  ...
  //  in and out time now the LCA will be the minimum between the two node x, y
  //  we can use a RMQ(sparse table) to find the LCA in O(1)
  ...
  const ll LOG = log2(n) + 1;
  vvl sparse_table(n, vl(LOG));

  vl depth(n);
  auto dfs = [&LOG, &sparse_table, &bosses, &depth](const auto& dfs,
                                                    const ll& node) -> void {
    for (auto& employee : bosses[node]) {
      depth[employee] = 1 + depth[node];
      sparse_table[employee][0] = node;
      Fo(j, 1, LOG) {
        sparse_table[employee][j] =
            sparse_table[sparse_table[employee][j - 1]][j - 1];
      }
      dfs(dfs, employee);
    }
  };
  ...
  auto lca = [&LOG, &depth, &sparse_table](ll x, ll y) {
    if (depth[x] < depth[y]) swap(x, y);

    ll k = depth[x] - depth[y];
    Fo(j, LOG - 1, -1) {
      //  if the j'th bit is set in k
      if (k & (1 << j)) x = sparse_table[x][j];
    }

    if (x == y)
      return x;  //  this is necessary, as in later we return the parent
