...
class TreeNode {
 public:
  ...
  TreeNode(T val) : val_(val) {}
};

inline void solve() {
  ll n;
  cin >> n;
  pbds_unordered_map<ll, TreeNode<ll>*> mp;

  //  building the tree
  {
    fo(i, n) {
      TreeNode<ll>* node = new TreeNode<ll>(i + 1);
      mp[i + 1] = node;
    }
    ...
    fo(i, n - 1) {
      ...
      mp[x]->children_.push_back(mp[i + 2]);
    }
  }

  TreeNode<ll>* root = mp[1];

  map<ll, ll> ans;
  auto dfs = [&ans](const auto& dfs, const TreeNode<ll>* const node) -> ll {
    ans[node->val_] += node->children_.size();
    for (auto& child : node->children_) ans[node->val_] += dfs(dfs, child);

    return ans[node->val_];
  };

  dfs(dfs, root);

  for (auto& node : ans) cout << node.second << ' ';
}
