 ...
 public:
  SegmentTree(const size_t& st_idx, const size_t& ed_idx, const vl& arr,
    ...
    if (left_most == right_most) {
      value_ = val[abs(arr[left_most])];

      if (arr[left_most] < 0) value_ *= -1;
...
inline void solve() {
  ...
  auto dfs = [&val, &gr, &euler, &in, &out, &cnt](
    ...
    out[node] = cnt++;
    //  when coming back, we remove that contribution, so putting negative to
    //  let the segment tree know
    euler.pb(-node);
  };
  dfs(dfs, 0, -1);
  ...
  while (q--) {
    ...
    if (type == 1) {
      ...
      //  negative as we don't want this path to be included
      seg_tree.point_update(out[s], -x);
    } else
      cout << seg_tree.range_query(in[0], in[s]) << '\n';
  }
}
