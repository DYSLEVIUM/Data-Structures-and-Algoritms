 ...
 private:
  ...
  void build() {
    ...
    for (size_t j = 1; j <= this->max_log; ++j) {
      for (size_t i = 0; i + (1 << j) <= this->_size; ++i)
        this->_table[i][j] = this->_fun(
            this->_table[i][j - 1], this->_table[i + (1 << (j - 1))][j - 1]);
    }
  }

 public:
  Sparse_Table(const vector<T>& data, T (*fun)(const T&, const T&),
               const T& identity, const bool& is_idempotent)
      : _size(data.size()),
        _fun(fun),
        _identity(identity),
        _is_idempotent(is_idempotent) {
  ...
  T query(size_t le, size_t ri) {
    ...
    if (this->_is_idempotent) {
      ...
      return this->_fun(this->_table[le][j],
                        this->_table[ri - (1 << j) + 1][j]);
    }
...
inline void solve() {
  ...
  auto mini = [](const ll& a, const ll& b) { return min(a, b); };

  Sparse_Table<ll> sp(x, mini, INT_MAX, true);
