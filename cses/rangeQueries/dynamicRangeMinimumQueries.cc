...
inline void solve() {
  ...
  auto update = [&st](const auto& update, const ll& lo, const ll& hi,
                      const ll& k, const ll& u, const ll& idx) {
    if (lo == hi) {
      st[idx] = u;
      return;
    }
    ...
    ll mid = lo + (hi - lo) / 2;
    if (k <= mid)
      update(update, lo, mid, k, u, 2 * idx + 1);
    else
      update(update, mid + 1, hi, k, u, 2 * idx + 2);

    st[idx] = min(st[2 * idx + 1], st[2 * idx + 2]);
  };

  auto query = [&st](const auto& query, const ll& lo, const ll& hi, const ll& a,
                     const ll& b, const ll& idx) {
    if (lo > b || hi < a) return LONG_LONG_MAX;
    if (lo >= a && hi <= b) return st[idx];

    ll mid = lo + (hi - lo) / 2;
    return min(query(query, lo, mid, a, b, 2 * idx + 1),
               query(query, mid + 1, hi, a, b, 2 * idx + 2));
  };
  ...
  while (q--) {
    ...
    if (t) {
      //  query
      cout << query(query, 0, n, x, y, 0) << '\n';
    } else {
      //  update
      update(update, 0, n, x, y + 1, 0);
    }
  }
}
