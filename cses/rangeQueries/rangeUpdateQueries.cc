...
inline void solve() {
  ...
  vl st(4 * n);
  vl delta(4 * n);
  auto build = [&st, &x](const auto& build, const ll& lo, const ll& hi,
                         const ll& idx) {
    if (lo == hi) {
      st[idx] = x[lo];
      return;
    ...
    ll mid = lo + (hi - lo) / 2;
    build(build, lo, mid, 2 * idx + 1);
    build(build, mid + 1, hi, 2 * idx + 2);

    st[idx] = st[2 * idx + 1] + st[2 * idx + 2];
  };

  auto range_update = [&st, &delta](const auto& range_update, const ll& lo,
                                    const ll& hi, const ll& a, const ll& b,
                                    const ll& u, const ll& idx) {
    if (lo > b || hi < a) return;
    if (lo >= a && hi <= b) {
      delta[idx] += u;
      return;
    ...
    ll mid = lo + (hi - lo) / 2;
    range_update(range_update, lo, mid, a, b, u, 2 * idx + 1);
    range_update(range_update, mid + 1, hi, a, b, u, 2 * idx + 2);
  };

  auto query = [&st, &delta](const auto& query, const ll& lo, const ll& hi,
                             const ll& a, const ll& b, const ll& idx) {
    if (lo > b || hi < a) return 0LL;
    if (lo >= a && hi <= b) return st[idx] + delta[idx];

    ll mid = lo + (hi - lo) / 2;
    return query(query, lo, mid, a, b, 2 * idx + 1) +
           query(query, mid + 1, hi, a, b, 2 * idx + 2) + delta[idx];
  };
  ...
  while (q--) {
    ...
    if (t) {
      ...
      --a, --b;
      range_update(range_update, 0, n, a, b, u, 0);
    }
  }
}
