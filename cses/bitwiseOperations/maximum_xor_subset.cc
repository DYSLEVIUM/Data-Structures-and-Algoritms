...
inline void solve() {
    ...
    // Gaussian Elimination (also known as a Linear Basis) over the field GF(2)
    vl basis(32, 0);
    auto basis_insert = [&](ll mask) {
        for (ll i = 31; i >= 0; --i) {
            // ith bit is 0, doesn't contribute to basis
            if (!(mask & (1 << i))) {
                continue;
            }
    ...
    //     if (maxx & (1 << i) || !basis[i]) {  // if num already has that bit
    //     set
    //                                          // or basis doesn't contribute
    //         continue;
    //     }
