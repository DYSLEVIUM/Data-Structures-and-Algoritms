...
// https://gemini.google.com/share/eb75fc50e109
inline void solve() {
    ll n, k;
    cin >> n >> k;
    ...
    // We have already printed 1 sum (the base_sum), so we need k-1 more.
    fo(i, k - 1) {
        auto [val, idx] = pq.top();
        pq.pop();

        // Print the calculated sum (Base + current non-negative subset sum)
        cout << " " << (base_sum + val);

        // If there are more elements in the array to process
        if (idx + 1 < n) {
            ll next_element = a[idx + 1];

            // Branch 1: Include the next element extending the current subset
            // New Sum = Current Sum + a[idx+1]
            pq.push({val + next_element, idx + 1});

            // Branch 2: Replace the last element with the next element
            // New Sum = Current Sum - a[idx] + a[idx+1]
            ll last_element = a[idx];
            pq.push({val - last_element + next_element, idx + 1});
        }
    }
}
