...
inline void solve() {
  ...
  auto dijkstra = [](const vector<vpl>& gr, const ll& stNode = 1) {
    ...
    while (!pq.empty()) {
      ...
      if (dist[node] < minDist) continue;

      for (auto neighbour : gr[node]) {
        ll newDist = dist[node] + neighbour.S;

        if (newDist < dist[neighbour.F]) {
          prev[neighbour.F] = node;
          dist[neighbour.F] = newDist;
          pq.emplace(newDist, neighbour.F);
        }
      }
    }
  ...
  auto findShortestPath = [&dijkstra](const vector<vpl>& gr, const ll& endNode) {
    pair<vl, vl> res = dijkstra(gr);

    vl path;

    if (res.F[endNode] == LONG_LONG_MAX) {
      path.eb(-1);
      return path;
    }
  ...
  vl path = findShortestPath(adjL, n);
