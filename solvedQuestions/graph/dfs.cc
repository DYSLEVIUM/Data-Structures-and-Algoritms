...
inline void solve() {
  ...
  adjList.emplace_back(vl{2, 5, 4});
  adjList.emplace_back(vl{0, 1, 3});
  adjList.emplace_back(vl{3});

  auto printGraph = [](const vvl& adjList) {
    fo(i, adjList.size()) {
      cout << i + 1 << " -> ";
      fo(j, adjList[i].size()) cout << adjList[i][j] << ' ';
      cout << '\n';
    }
  };

  vl dfsArr(0);
  vl visited(adjList.size(), false);
  auto dfs = [&dfsArr, &visited](const vvl& gr, const ll& idx, const auto& dfs) {
    if (visited[idx]) return;

    visited[idx] = true;
    dfsArr.eb(idx);

    fo(i, gr[idx].size()) dfs(gr, gr[idx][i], dfs);
  };

  dfs(adjList, 0, dfs);

  tr(it, dfsArr) cout << *it << ' ';
}
