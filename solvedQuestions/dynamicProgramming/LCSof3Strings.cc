...
inline void solve() {
    auto lcs = [](string s1, string s2, string s3) -> ll {
        ll*** dp = new ll**[s1.length() + 1];

        for (ll i{}; i <= s1.length(); ++i) {
            dp[i] = new ll*[s2.length() + 1];

            for (ll j{}; j <= s2.length() + 1; ++j)
                dp[i][j] = new ll[s3.length() + 1];
        }

        for (ll i{}; i <= s1.length(); ++i) {
            for (ll j{}; j <= s2.length(); ++j) {
                for (ll k{}; k <= s3.length(); ++k) {
                    if (i == 0 || j == 0 || k == 0)
                        dp[i][j][k] = 0;
