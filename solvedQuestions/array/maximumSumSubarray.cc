...
int maximumSubarraySum1(int* arr, int n) {
    int ans = arr[0];  //  choosing atleast one element from array

    fo(i, n) {
        fo(j, n) {
            int sum = 0;
            Fo(k, i, j + 1) {  //j+1 cause j can be 0 to n-2
                sum += arr[k];
                ans = max(sum, ans);
            }
        }
...
//  kandane's algorithm
int maximumSubarraySum3(int* arr, int n) {
    int ans = arr[0];  //  choosing atleast one element from array
    int sum = arr[0];  //choosing atleast one element from array

    Fo(i, 1, n) {  //  starting from 2nd element as first element is already chosen
...
inline void solve() {
    ...
    cout << maximumSubarraySum3(arr, n);
}
