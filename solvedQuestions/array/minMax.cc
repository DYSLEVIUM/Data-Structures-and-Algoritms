...
Pair getMinMax2(int arr[], int low, int high) {
    ...
    if (low == high) {  //only one element is there
        minmax.min = arr[low];
        minmax.max = arr[low];
        return minmax;
    }
    ...
    mid = low + (high - low) / 2;
    mml = getMinMax2(arr, low, mid);       //  left half
    mmr = getMinMax2(arr, mid + 1, high);  //   right half
...
inline void solve() {
    ...
    cout << getMinMax1(arr, n).min << ' ' << getMinMax1(arr, n).max << '\n';

    cout << getMinMax2(arr, 0, n - 1).min << ' ' << getMinMax2(arr, 0, n - 1).max << '\n';
}
