...
inline void solve() {
  ...
  auto preOrderRecur = [](TreeNode* node, const auto& preOrderRecur) {
    if (node == nullptr) return;

    cout << node->data << ' ';
    preOrderRecur(node->left, preOrderRecur);
    preOrderRecur(node->right, preOrderRecur);
  };

  auto preOrderIter = [](TreeNode* node) {
    if (node == nullptr) return;
  ...
  preOrderRecur(root, preOrderRecur);
  cout << '\n';
  preOrderIter(root);
}
