...
inline void solve() {
  ...
  auto inOrderRecur = [](TreeNode* node, const auto& inOrderRecur) {
    if (node == nullptr) return;

    inOrderRecur(node->left, inOrderRecur);
    cout << node->data << ' ';
    inOrderRecur(node->right, inOrderRecur);
  };

  auto inOrderIter = [](TreeNode* node) {
    if (node == nullptr) return;
  ...
  inOrderRecur(root, inOrderRecur);
  cout << '\n';
  inOrderIter(root);
}
