...
class tree_node {
 public:
  ...
  tree_node(const ll& v) : val(v), left(nullptr), right(nullptr) {}
};

inline void solve() {
  tree_node* t = new tree_node(5);
  t->left = new tree_node(3);
  t->right = new tree_node(6);
  t->left->left = new tree_node(2);
  t->left->right = new tree_node(4);

  auto level_order = [](const tree_node* const node) {
    if (!node) return;

    queue<const tree_node*> qu;
    qu.push(node);

    while (!qu.empty()) {
      const tree_node* fr = qu.front();
      qu.pop();
  ...
  cout << "Level Order before mirror\n";
  level_order(t);
  cout << '\n';

  auto mirror = [](const auto& mirror, tree_node* const node) {
    if (!node) return;

    swap(node->left, node->right);

    if (node->left) mirror(mirror, node->left);
    if (node->right) mirror(mirror, node->right);
  };

  mirror(mirror, t);

  cout << "Level Order after mirror\n";
  level_order(t);
}
