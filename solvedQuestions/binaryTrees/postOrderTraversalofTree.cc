...
inline void solve() {
  ...
  auto postOrderRecur = [](TreeNode* node, const auto& postOrderRecur) {
    if (node == nullptr) return;

    postOrderRecur(node->left, postOrderRecur);
    postOrderRecur(node->right, postOrderRecur);
    cout << node->data << ' ';
  };

  auto postOrderIter = [](TreeNode* node) {
    if (node == nullptr) return;
  ...
  postOrderRecur(root, postOrderRecur);
  cout << '\n';
  postOrderIter(root);
}
