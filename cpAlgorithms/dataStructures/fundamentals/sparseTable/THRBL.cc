...
inline void solve() {
  ...
  Sparse_Table<ll> sp(h, maxi, INT_MIN, true);
  ...
  while (m--) {
    ...
    //  if we are sending from right to left
    if (a > b)
      swap(a, b);
    else if (a == b)
      ++ans;
    else
      //  if the leftmost point is the maximum of the range, we can catapult the ball
      //  right side is b-1 as it is said between them 🤦‍♂️
      ans += (sp.query(a, b - 1) == h[a]);
  }
