...
inline void solve() {
  auto extendedEuclideanRecur = [](ll a, ll b, ll& x, ll& y, auto&& extendedEuclideanRecur) {
    if (b == 0) {
      x = 1;
      y = 0;
      return a;
    ...
    ll x1, y1;
    ll d = extendedEuclideanRecur(b, a % b, x1, y1, extendedEuclideanRecur);
    x = y1;
    y = x1 - y1 * (a / b);
    return d;
  };

  auto extendedEuclideanIter = [](ll a, ll b, ll& x, ll& y) {
    x = 1, y = 0;
